...
For a remote connector, use [Customize > Connectors > Add custom connector](https://support.claude.com/en/articles/11175166-get-started-with-custom-connectors-using-remote-mcp). If its current UI only accepts a URL, SerpApi supports `https://mcp.serpapi.com/your_key_here/mcp`. Explain that the URL contains the secret and have the user enter it directly in the connector UI. Never put the completed URL in chat, repository files, or shell arguments. Do not use a local JSON file to configure Claude's remote connector, which runs in the cloud.
